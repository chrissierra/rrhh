...
function insert_mysql_sucursales($nombre, $direccion, $comuna, $ciudad, $usuario, $latitud, $longitud){

    try {
        $tabla="sucursales";
    $this->conexion();
    $pdo = $this->dbh->prepare('INSERT INTO '.$tabla.' SET nombre=:nombre, direccion=:direccion, comuna=:comuna, ciudad=:ciudad, usuario=:usuario, latitud=:latitud, longitud=:longitud');
    ...
    $pdo->bindValue(':latitud', $latitud);
    $pdo->bindValue(':longitud', $longitud);
    $pdo->execute();
    $this->dbh = null;
    echo $this::EXITO;
    } catch (PDOException $e) {
